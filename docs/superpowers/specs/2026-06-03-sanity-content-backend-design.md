...
目标是把随笔、书影音、时间线从本地 MDX 文件管理，迁移到独立部署的 Sanity Studio 后台。Astro 前台继续负责展示内容和保持现有视觉体验，Sanity Studio 负责新增、编辑、上传封面、草稿和发布。内容页面应动态读取 Sanity 已发布内容，让后台发布后的内容在前台刷新后尽快可见。
...
Astro 前台部署平台需要支持服务端渲染或按请求渲染。v0.2.0 不采用纯静态内容输出作为默认方案。
...
- 页面请求时从 Sanity 查询已发布内容。
- Sanity 发布新内容后，访客刷新对应页面即可看到更新。
- 不把 webhook 触发重新部署作为默认更新方式。
- 不使用纯浏览器端拉取作为主要内容渲染方式，避免影响首屏阅读和 SEO。
- 可以根据部署平台能力增加短缓存，但缓存不能让新发布内容长时间不可见。

如果未来为了性能需要缓存，应优先使用短 TTL、按页面再验证或平台级动态缓存，而不是回到纯静态生成。
...
如果只读取公开已发布内容，前台不应需要暴露私密 token。Sanity 客户端应优先保证发布后的内容新鲜度，避免长期 CDN 缓存让前台持续显示旧内容。
...
- Sanity 发布新内容后，不需要重新部署前台即可在刷新页面后看到更新。
- 草稿内容不会在前台展示。
